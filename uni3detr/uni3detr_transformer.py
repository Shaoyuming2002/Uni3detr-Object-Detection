...
class Uni3DETRTransformerDecoder(TransformerLayerSequence):
    ...
    # # 如果是 [B, Nq, D] 格式 → 转置成 [Nq, B, D]
    #         query = query.permute(1, 0, 2)
        print("Inside transformer decoder input:")
        # print("query:", query)
        # print("key:", key)
...
class UniCrossAtten(BaseModule):
    ...
    def forward(self,
        ...
        embed_uni = []
        print("pts_value.shape:", pts_value.shape)  
        print("reference_points_voxel.shape:", reference_points_voxel.shape)
        # shape: (N, C, D, H, W)
        # without height
        if len(pts_value.shape) == 4:
            reference_points_voxel = reference_points_voxel.reshape(-1,1,*reference_points_voxel.shape[-2:])[...,:2]
        else:
            pts_value = pts_value.reshape(-1, *pts_value.shape[2:])
            reference_points_voxel = reference_points_voxel.reshape(-1, 1, 1, *reference_points_voxel.shape[-2:])
